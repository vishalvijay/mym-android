...
public class QuizDB {
	...
	public static void setUpTable(SQLiteDatabase db) {
		...
		inserQuiz(
				db,
				19,
				"Clearing and settlement operations of the NSE are carried out by?",
				"National Security Depository Ltd.",
				"National Security Clearing Co-operation",
				"State Bank of India", "By the exchange itself", 1,
				Quiz.TYPE_4_OPTIONS);
		...
		inserQuiz(
				db,
				23,
				"Mumbai stock exchange was recognised on a permanent basis in ",
				"1956", "1957", "1950", "1958", 1, Quiz.TYPE_4_OPTIONS);

		inserQuiz(db, 24, "The BSE was established in? ", "1875", "1873",
		...
		inserQuiz(db, 26, "A security giving you the right to sell is a? ",
				"Swaption", "Call option", "Put option",
				"Short future contract", 2, Quiz.TYPE_4_OPTIONS);

		inserQuiz(db, 27, "The BSE was established as a? ",
				"Limited liability firm", "A partnership firm", "A company",
				"Voluntary non-profit Organization", 3, Quiz.TYPE_4_OPTIONS);

		inserQuiz(db, 28, "The SEBI was established in? ", "1990", "1988",
				"1989", "1992", 1, Quiz.TYPE_4_OPTIONS);

		inserQuiz(
				db,
				29,
				"What kind of measure of a company’s performance and conditions do ratios provide?",
				"Absolute", "Definitive", "Gross", "Relative", 3,
				Quiz.TYPE_4_OPTIONS);

		inserQuiz(db, 30,
				"A person who combines derivatives with a business risk is a?",
				"Hedger", "Speculator", "Spreader", "Stock broker", 0,
				Quiz.TYPE_4_OPTIONS);

		inserQuiz(db, 31,
				"Insider trading id reported can be investigated by?", "SEBI",
				"FMC", "MCA", "RBI", 0, Quiz.TYPE_4_OPTIONS);

		inserQuiz(db, 32,
				"The credit of pledged securities remains in the account of?",
				"The pledgor", "The pledge", "Both", "None of the above", 0,
				Quiz.TYPE_4_OPTIONS);

		inserQuiz(db, 33,
				"The depository account that an investor opens is called? ",
				"Broker account", "Intermediary account",
				"Beneficiary account", "Clearing member account", 2,
				Quiz.TYPE_4_OPTIONS);
		...
		inserQuiz(
				db,
				34,
				"An investor holding shares in demat form will get his bonus entitlement in?",
				"Demat form", "Physical form",
